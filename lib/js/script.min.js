...
function onloadCallback() {
    grecaptcha.render("faucetClaimCaptcha", {
        sitekey: "6LeE5SkTAAAAAC1xYDh6mGJIfSZGTiVHcNjfqWms",
...
function startNewGame() {
  // APP ID
    if (!connected) return $("#theDimmer").html('Error: Not connected!<br><a href="https://www.moneypot.com/oauth/authorize?app_id=2279&response_type=token&state=Meh&redirect_uri=http://doubloon.co">Click here to login</a>'), void $("#game_left").dimmer("show");
    ...
    if (game.tilesClicked = 0, houseEdge = .028, odds = (25 - game.bombs - game.tilesClicked) / (25 - game.tilesClicked), next = game.bet * ((1 - houseEdge) / odds) - game.bet, game = {
            status: "IN_PROGRESS",
            profit: 0,
            bet: game.bet,
            bombs: game.bombs,
        ...
        $(this).click(function() {
            if ("IN_PROGRESS" == game.status) {
                ...
                if ($(".tile").each(function(a) {
					...
					if (game.stake > 2000) {
					var jpm = 2000000000 / (game.stake);
					var jpchance = (game.stake) / 2000 * 3435;
					} else {
					var jpm = 1000000;
					var jpchance = 3435;
					}
...
function BotStartStop()
{
if(BotRunning == true)
{
var _settings = {
    baseBet: 0.1, // 1 bit. Cannot be lower than 0.01.
    bombs: 4, // 3 bombs. Cannot be lower than 1 or higher than 24.
    multiply: 6.66, // Multiply the previous stake by 8 times if lost.
...
function placeBet(){
    ...
    odds = ( (25-_settings.bombs-tilesClicked) / (25-tilesClicked) );
	if (bet > 2000) {
		var jpm = 2000000000 / (bet);
		var jpchance = (bet) / 2000 * 2048;
	} else {
		var jpm = 1000000;
		var jpchance = 2048;
	}
    ...
    $.ajax({
        ...
        data: JSON.stringify({
            ...
            "payouts": [
                ...
                {from: 0, to: jpchance, value: (jpm) * (bet)}
            ]
        }),
        dataType: "json",
...
function escapeHTML(a) {
    a += "";
    for (var b = "", c = 0; c < a.length; c++) b += "<" === a[c] ? "&lt;" : ">" === a[c] ? "&gt;" : "'" === a[c] ? "&#39;" : '"' === a[c] ? "&#34;" : "%" === a[c] ? "&#37;" : "\\" === a[c] ? "&#92;" : "Â¢" === a[c] ? "&#162;" : "Â¼" === a[c] ? "&#188;" : "Â½" === a[c] ? "&#189;" : "Â¾" === a[c] ? "&#190;" : "#" === a[c] ? "&#35;" : "&" === a[c] && "#" === a[c + 1] ? "&#38;" : a[c];
    return b
}
...
$("#bet").val(game.bet), $("#bombsButtons .button").each(function(a) {
    ...
    socket.on("connect", function() {
        ...
        socket.emit("auth", a, function(a, b) {
            ...
            }), $.getJSON("https://api.moneypot.com/v1/list-bets?access_token=" + access_token + "&app_id=2279&wager=4500000&limit=11&order_by=desc", function(a) {
    ...
    var a = "https://www.moneypot.com/dialog/deposit?app_id=2279",
        b = "manage-auth",
        c = "width=420,height=350,left=100,top=100",
        d = window.open(a, b, c);
    d.focus()
}), $("#withdrawButton").click(function() {
    var a = "https://www.moneypot.com/dialog/withdraw?app_id=2279",
        b = "manage-auth",
        c = "width=420,height=350,left=100,top=100",
        d = window.open(a, b, c);
    d.focus()
